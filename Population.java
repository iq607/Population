		...
		while (true)
		{
			...
			else if (input == 6)
			{
				...
				if (matchingCities.size() > 50)
					matchingCities = subList(matchingCities.size()-50, matchingCities.size(), matchingCities);
			...
			else if (input == 3)
			{
				...
				matchingCities = subList(0, 50, matchingCities);
			}
			else if (input == 2)
			{
			...
			System.out.println("\nElapsed Time "+(int)(endMillisec-startMillisec)+" millseconds\n");
		}
		System.out.println("\nThanks for using Population!");
	}
	
	/**	substring but for arraylist
	 * @param start		starting index
	 * @param end 		ending index
	 * @return			subarray
	 */
	public ArrayList<City> subList(int start, int end, ArrayList<City> arr)
	{
		ArrayList<City> subarray = new ArrayList<City>();
